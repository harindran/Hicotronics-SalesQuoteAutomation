...
Namespace SalesQuoteAutomation
    ...
    Public Class clsGlobalMethods
        ...
        Public Function default_series(ByVal objectid As String, ByVal docdate As String)
            Try
                objrs = objaddon.objcompany.GetBusinessObject(SAPbobsCOM.BoObjectTypes.BoRecordset)
                Dim FDate As Date = Date.ParseExact(docdate, "dd-MM-yy", System.Globalization.DateTimeFormatInfo.InvariantInfo)
                strsql = " CALL ""MIPL_GetDefaultSeries"" ('" & objectid.ToString & "','" & objaddon.objcompany.UserName & "','" & FDate.ToString("yyyyMMdd") & "')"
                objrs.DoQuery(strsql)
...
addrow:
        ...
        Public Sub LoadSeries(ByVal objform As SAPbouiCOM.Form, ByVal DBSource As SAPbouiCOM.DBDataSource)
            Try
                ...
                ComboBox0.ValidValues.LoadSeries("SQUO", SAPbouiCOM.BoSeriesMode.sf_Add)
                ComboBox0.Select(0, SAPbouiCOM.BoSearchKey.psk_Index)
                DBSource.SetValue("DocNum", 0, GetDocNum("SQUO", CInt(ComboBox0.Selected.Value)))
            Catch ex As Exception
